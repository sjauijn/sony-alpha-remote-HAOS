...
class SonyCameraConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            self._abort_if_unique_id_configured()
            self._mac = mac
            self._name = discovered.get(mac) or mac
            self.context["title_placeholders"] = {"name": self._name}
            return await self.async_step_pair()
    ...
    async def async_step_pair_progress(
        ...
        The frontend does not automatically re-poll the flow when the
        progress task finishes on its own -- it needs the flow to be
        re-configured to notice, so a done-callback nudges it forward.
        """
        if self._pair_task is None:
            self._pair_task = self.hass.async_create_task(self._async_pair_task())
            self._pair_task.add_done_callback(self._pair_task_done)
    ...
    @callback
    def _pair_task_done(self, _task: asyncio.Task[None]) -> None:
