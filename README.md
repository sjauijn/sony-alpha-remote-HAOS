...
адаптерах). В этом случае как запасной вариант остаётся ручной пейринг
через `bluetoothctl` на хосте (см. историю обсуждения) — но для
большинства адаптеров, поддерживаемых Home Assistant OS, штатный путь
через UI должен работать.
